...
export default function Home() {
    ...
    const imprimir = item.length > 0 ? (item) : (users);

    function HandleName() {
        const filtros = users.map(item => item.name).filter(item => item.first == storageName);
        const filtrosName = filtros.find(item => item.first);
        setItem(filtrosName)
        if( storageName == ""){
        return alert ('Insira um nome')
        }else {
                setStorageName("");    
                console.log(filtrosName)
            }  
        }
        ...
        <>
            <Container>
                <Box>
                   ...
                   {imprimir.map( (user) => <ContainerUsers key={user.picture}> 
                            <div>
